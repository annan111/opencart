...
$(document).ready(function(){ 
	...
	$(function() {
		$("#bt_slide_list tbody").sortable({ opacity: 0.8, cursor: 'move', update: function() {
			...
			$.post("index.php?route=extension/module/boss_revolutionslider/updateSortSlide&token=<?php echo $token; ?>", order, function(theResponse){
...
function deleteSlide(id,slider_id,module_id){
	if (!confirm('<?php echo $text_confirm; ?>')) {
		...
		$.ajax({
			url: 'index.php?route=extension/module/boss_revolutionslider/deleteSlide&token=<?php echo $token; ?>&slide_id='+id+'&slider_id='+slider_id+'&module_id='+module_id,
			async:false,
			type: 'post',
			dataType: 'json',
...
function copySlide(id,slider_id,module_id){
	$.ajax({
		url: 'index.php?route=extension/module/boss_revolutionslider/copySlide&token=<?php echo $token; ?>&slide_id='+id+'&slider_id='+slider_id+'&module_id='+module_id,
		async:false,
		type: 'post',
		dataType: 'json',
		success: function(json) {
