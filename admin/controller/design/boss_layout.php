...
class ControllerDesignBossLayout extends Controller {
	...
	protected function getList() {
		...
		$data['module_manager'] = $this->url->link('extension/extension', 'token=' . $this->session->data['token'], 'SSL');
		...
		foreach ($extensions as $code) {
			...
			if(empty($modules)){
				$module_data[] = array(
					...
					'code' => $code,
					'edit'      => $this->url->link('extension/module/' . $code . '', 'token=' . $this->session->data['token'], 'SSL'),
					'status' => false
				);
			}else{
				foreach ($modules as $module) {
						...
						$module_data[] = array(
							...
							'edit'      => $this->url->link('extension/module/' . $code, 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL'),
		...
		if (isset($this->request->post['layout_module'])) {
			...
			foreach ($layout_modules as $layout_module) {
					...
					if(!isset($part[0])){
						$href = $this->url->link('extension/module/' . $layout_module['code'], 'token=' . $this->session->data['token'], 'SSL');
					}elseif (isset($part[0]) && !isset($part[1])) {						
						$href = $this->url->link('extension/module/' . $part[0], 'token=' . $this->session->data['token'], 'SSL');
					}else{						
						$href = $this->url->link('extension/module/' . $part[0], 'token=' . $this->session->data['token'] . '&module_id=' . $part[1], 'SSL');
					}
