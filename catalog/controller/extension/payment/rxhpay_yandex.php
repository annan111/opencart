...
class ControllerExtensionPaymentrxhpayyandex extends Controller {
	...
	public function index() {
		...
		return $this->load->view('extension/payment/rxhpay_yandex', $data);
	}
	...
	public function confirm()
	{
		...
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('rxhpay_yandex_unpaid_id'), 'Yandex Order submitted', true);
	}
	public function getPayType(){
		$this->session->data['payType']=$this->request->get['payType'];
	}
	...
	//支付接口参数组合,并提交rxhpay_yandex进行支付
	public function torxhpay_yandex() {
		$data = array();
		$this->load->model('checkout/order');
		$order_info= $this->model_checkout_order->getOrder($this->session->data['order_id']);
		//基础数据
		$url=$this->config->get('rxhpay_yandex_transaction_url');
		if($url){
			$data['action'] = $url;
		}else{
			$data['action'] = "http://www.rucnmall.com/payment/cross_pay";
		}
		$merNo= $this->config->get('rxhpay_yandex_merchant');
		...
		$payMode="Yandex";
		//支付类型:
		$payType=$this->session->data['payType'];
		...
		$fee='0';
		//其他
		$returnURL = $this->url->link('extension/payment/rxhpay_yandex/callback');
		$NotifyURL =$this->url->link('extension/payment/rxhpay_yandex/notify');
		$UserAgent =$_SERVER['HTTP_USER_AGENT'];
		$md5Key = $this->config->get('rxhpay_yandex_md5key');
			...
			if($verifyCode == $verify_md5Info){
				...
				if(isset($order['order_status_id']) && ($order['order_status_id'] != $this->pay_state($verify_succeed))){
					if($verify_succeed == '0'){
						...
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_yandex_approved_id'), $verify_result, true);
					}else if($verify_succeed == '1'){
						//支付失败
						if(empty($verify_result)) $verify_result = 'Payment failure!';
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_yandex_declined_id'), $verify_result, true);
					}else if($verify_succeed == '2' || $verify_succeed == '98'){
						//支付中或者支付超时
						if(empty($verify_result)) $verify_result = 'Bank timeout!';
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_yandex_pending_id'), $verify_result, true);
					}
				}
			}
			echo 'succeed';
			...
			if(isset($order['order_status_id']) && ($order['order_status_id'] != $this->pay_state($verify_succeed))){
				if($verifyCode == $verify_md5Info && $verify_succeed == '0'){
					...
					if($verify_succeed == "error"){
						...
						$this->model_checkout_order->addOrderHistory($verify_orderNo,$this->config->get('rxhpay_yandex_error_id'),$verify_result, true);
					}else{
						//签名不正确
						if(empty($verify_result)) $verify_result = '<font color="green">Signature incorrect!</font>';
						$data['text_result'] = $verify_result;
						//签名不正确数据加入订单历史记录
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_yandex_abnormal_id'), $verify_result, true);
					}
				}else{
					//签名不正确
