...
class ControllerExtensionPaymentRxhPay extends Controller {
	...
	public function index() {
		...
		return $this->load->view('extension/payment/rxhpay', $data);
	}
	...
	public function confirm()
	{
		...
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('rxhpay_unpaid_id'), '', true);
	}
	...
	public function torxhpay() {
		$this->load->model('checkout/order');
		$order_info= $this->model_checkout_order->getOrder($this->session->data['order_id']);

		$url=$this->config->get('rxhpay_transaction_url');
		if($url){
			$data['action'] = $url;
		}else{
			$data['action'] = "http://pay.rxhpay.com/cross_pay";			
		}
		$merNo= $this->config->get('rxhpay_merchant');
		...
		$fee='0';
		//其他
	  	$returnURL = $this->url->link('extension/payment/rxhpay/callback');
	  	$NotifyURL =$this->url->link('extension/payment/rxhpay/notify');
	  	$UserAgent =$_SERVER['HTTP_USER_AGENT'];
		$md5Key = $this->config->get('rxhpay_md5key');
			...
			if($verifyCode == $verify_md5Info){
				...
				if(isset($order['order_status_id']) && ($order['order_status_id'] != $this->pay_state($verify_succeed))){
					if($verify_succeed == '0'){
						...
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_approved_id'), $verify_result, true);
					}else if($verify_succeed == '1'){
						//支付失败
						if(empty($verify_result)) $verify_result = 'Payment failure!';
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_declined_id'), $verify_result, true);
					}else if($verify_succeed == '2' || $verify_succeed == '98'){
						//支付中或者支付超时
						if(empty($verify_result)) $verify_result = 'Bank timeout!';
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_pending_id'), $verify_result, true);
					}
				}
			}
			echo 'succeed';
	...
	public function pay_state($succed){
		if($succed==0){
			return $this->config->get('rxhpay_approved_id');
		}else if($succed==1){
			return $this->config->get('rxhpay_declined_id');
		}else if($succed==2 || $succed==98){
			return $this->config->get('rxhpay_pending_id');
		}else if($succed=='error'){
			return $this->config->get('rxhpay_error_id');
		}else{
			return $this->config->get('rxhpay_abnormal_id');
		}
	}
	...
	public function callback() {
		...
		if($this->request->post){
			$verify_orderNo=!empty($this->request->post['billNo'])? $this->request->post['billNo']: $this->session->data['order_id'];
			$verify_currency=!empty($this->request->post['currency'])? $this->request->post['currency']:'';
			$verify_amount=isset($this->request->post['amount'])? $this->request->post['amount']:'';
			$verify_succeed=isset($this->request->post['succeed'])? $this->request->post['succeed']:'';
			$verify_result=isset($this->request->post['result'])? $this->request->post['result']:'';
			$verify_md5Info=isset($this->request->post['md5Info'])? strtoupper($this->request->post['md5Info']):'';
			...
			if(isset($order['order_status_id']) && ($order['order_status_id'] != $this->pay_state($verify_succeed))){
				if($verifyCode == $verify_md5Info && $verify_succeed == '0'){
					...
					if($verify_succeed == "error"){
						...
						$this->model_checkout_order->addOrderHistory($verify_orderNo,$this->config->get('rxhpay_error_id'),$verify_result, true);
					}else{
						//签名不正确
						if(empty($verify_result)) $verify_result = '<font color="green">Signature incorrect!</font>';
						$data['text_result'] = $verify_result;
						//签名不正确数据加入订单历史记录
						$this->model_checkout_order->addOrderHistory($verify_orderNo, $this->config->get('rxhpay_abnormal_id'), $verify_result, true);
					}
				}else{
					//签名不正确
